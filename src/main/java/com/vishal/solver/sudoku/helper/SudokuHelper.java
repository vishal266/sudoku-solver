...
public class SudokuHelper {
    ...
    public boolean isValid(int[][] matrix, int i, int j, int val) {
        ...
        for(int x = 0; x< SudokuBoard.SUDOKU_BOARD_SIZE; x++) {
            if(x != i && matrix[x][j] == val) {
                return false;
            }
        }
        for(int y = 0; y< SudokuBoard.SUDOKU_BOARD_SIZE; y++) {
    ...
    public Pair<Integer, Integer> getNextRowColToProcess(SearchNodeImpl searchNode) throws SudokuBoardNullException,
        ...
        if(matrix.length != SudokuBoard.SUDOKU_BOARD_SIZE || matrix[0].length != SudokuBoard.SUDOKU_BOARD_SIZE) {
        ...
        if(matrix[r][c] < 0 || matrix[r][c] > SudokuBoard.SUDOKU_BOARD_SIZE) {
